...
def process_sample(forward, reverse, base_name, bowtie2_index, kraken_db, output_dir, threads,
    ...
    """
    Process a sample through trimming (if needed), optional host depletion or de novo assembly, and Kraken2 classification.
    
    Parameters:
      forward (str): Path to forward FASTQ.
      reverse (str): Path to reverse FASTQ.
      base_name (str): Base sample name.
      bowtie2_index (str): Path prefix to Bowtie2 index.
      kraken_db (str): Path to Kraken2 database.
      output_dir (str): Directory for outputs.
      threads (int): Number of threads.
      run_bowtie (bool): If True, run host depletion.
      use_precomputed_reports (bool): If True, skip processing and use existing report.
      use_assembly (bool): If True, perform de novo assembly.
      skip_preprocessing (bool): If True, skip trimming/host depletion and use assembly directly.
      
    Returns:
      str: Path to Kraken2 report for the sample or None on error.
    """
    try:
        ...
        if skip_preprocessing:
            ...
            # Run Trimmomatic if outputs do not exist
            ...
            # Optionally run Bowtie2 for host depletion
            if run_bowtie:
        ...
        # Run Kraken2 if report not already present
        kraken_report = os.path.join(output_dir, f"{base_name}_kraken_report.txt")
        if not os.path.exists(kraken_report):
            ...
            kraken_report = run_kraken2(input_r1, input_r2, base_name, kraken_db, output_dir, threads)
        else:
